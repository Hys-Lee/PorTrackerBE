...
class FinanceServiceTest {
        ...
        void getAndContributeStats_CacheMiss_Success() throws Exception {
                ...
                List<TransactionDto> mockData = List
                                .of(new TransactionDto("2026-02-05", "food", "lunch", 10000L, ""));

                Map<String, Object> mockProfile = Map.of(ProfileSchema.AGE_GROUP, "30s",
                                ProfileSchema.JOB_TYPE, "developer");
                ...
                // 모킹 실행
                List<TransactionDto> result = financeService.getAndContributeStats(accessToken,
                                userId, sheetId, false);
                ...
                // 내부 로직 확인
                verify(statisticsService, times(1)).anonymized(eq(userId), anyList(),
                                eq(mockProfile));
