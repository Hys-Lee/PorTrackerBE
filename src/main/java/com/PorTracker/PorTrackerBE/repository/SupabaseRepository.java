...
public class SupabaseRepository {
    ...
    public void upsertCategoryStats(List<?> statsList) {
        ...
        String onConflictColumns =
                String.join(
                        ",",
                        StatsSchema.HASHED_ID,
                        StatsSchema.YEAR_MONTH,
                        StatsSchema.CATEGORY,
                        StatsSchema.AGE_GROUP,
                        ...
                        response ->
                                response.bodyToMono(String.class)
                                        .flatMap(
                                                body -> {
                                                    System.err.println(
                                                            "Supabase error msg: " + body);
                                                    return Mono.error(
                                                            new RuntimeException(
                                                                    "Supabase API Error: " + body));
                                                }))
                .toBodilessEntity()
                .block();
    }
    ...
    public List<GroupAverageResponse> getGroupAverages(
            String ageGroup, String jobType, String period) {
        return getWebClient()
                .get()
                .uri(
                        uriBuilder ->
                                uriBuilder
                                        ...
                                        .queryParam(StatsSchema.YEAR_MONTH, "eq." + period)
                                        .build())
                .retrieve()
