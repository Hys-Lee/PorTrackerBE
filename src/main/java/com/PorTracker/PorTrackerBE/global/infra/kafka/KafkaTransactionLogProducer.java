...
public class KafkaTransactionLogProducer {
     ...
     */
    public void sendTransactionLog(String userId, String actionType, String sqlQuery, Object[] params) {
        try {
            ...
            log.error("[KafkaWAL] Failed to write commit log synchronously for user: {}", userId, e);
            throw new RuntimeException("WAL 영속화 실패 - 트랜잭션을 보장할 수 없습니다.", e);
        }
    }

    public void sendServiceEvent(String userId, String serviceName, String methodName, Object[] args) {
        try {
            Map<String, Object> messageMap = new HashMap<>();
            messageMap.put("userId", userId);
