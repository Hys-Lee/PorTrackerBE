...
public class RateLimitingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (path.startsWith("/swagger-ui") || path.startsWith("/v3/api-docs") || path.startsWith("/swagger-resource") || path.startsWith("/api/v1/public")) {
        ...
        String limitKey = (userId != null) ? "ratelimit:user:" + userId : "ratelimit:ip:" + request.getRemoteAddr();
        ...
        try {
            ...
            if (currentCount != null && currentCount > MAX_REQUESTS_PER_MINUTE) {
                log.warn("[RateLimit] Limit exceeded for key: {}. Count: {}", limitKey, currentCount);
                response.setStatus(HttpStatus.TOO_MANY_REQUESTS.value());
                response.setContentType("application/json; charset=UTF-8");
                response.getWriter().write("{\"error\": \"Too Many Requests\", \"message\": \"요청 한도를 초과했습니다. 잠시 후 다시 시도해주세요.\"}");
                return;
            }
        } catch (Exception e) {
