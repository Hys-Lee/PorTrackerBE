...
public class MemoRepository {
    ...
    private static final String BASE_SELECT_SQL = String.format(
            ...
            // select
            SqliteSchema.COL_ID, SqliteSchema.COL_PUBLIC_ID, SqliteSchema.COL_CREATED_AT,
            // SqliteSchema.COL_UPDATED_AT, SqliteSchema.COL_DELETED_AT,
            SqliteSchema.COL_IMPORTANCE, SqliteSchema.COL_TITLE, SqliteSchema.COL_CONTENT,
            SqliteSchema.COL_EVALUATION, SqliteSchema.COL_DATE, SqliteSchema.COL_MEMO_TYPE,
            SqliteSchema.COL_ACTUAL_ID, SqliteSchema.COL_TARGET_ID, SqliteSchema.COL_PUBLIC_ID,
            actualPortfolioPublicIdName, SqliteSchema.COL_PUBLIC_ID, targetPortfolioPublicIdName,

            // from
            SqliteSchema.TABLE_MEMO, SqliteSchema.TABLE_ACTUAL_PORTFOLIO,
            SqliteSchema.COL_ACTUAL_ID, SqliteSchema.COL_ID, SqliteSchema.TABLE_TARGET_PORTFOLIO,
            SqliteSchema.COL_TARGET_ID, SqliteSchema.COL_ID);

    private final RowMapper<MemoRecord> memoMapper = (rs, rowNum) -> MemoRecord.builder()
            .id(rs.getLong(SqliteSchema.COL_ID)).publicId(rs.getString(SqliteSchema.COL_PUBLIC_ID))
            ...
            .targetPublicId(rs.getString(targetPortfolioPublicIdName)).build();
    ...
    public Optional<MemoRecord> findByPublicId(JdbcTemplate jdbcTemplate, String publicId) {
        String sql = BASE_SELECT_SQL + " WHERE " + "m." + SqliteSchema.COL_PUBLIC_ID + "=? AND "
                + "m." + SqliteSchema.COL_DELETED_AT + " IS NULL";
        return jdbcTemplate.query(sql, ps -> ps.setString(1, publicId), memoMapper).stream()
                .findFirst();
    }

    public Long save(JdbcTemplate jdbcTemplate, MemoCreateRequest request, String publicId,
            Long actualId, Long targetId) {
        // test
        ...
        String sql = String.format(
                ...
                SqliteSchema.TABLE_MEMO, SqliteSchema.COL_PUBLIC_ID, SqliteSchema.COL_IMPORTANCE,
                SqliteSchema.COL_TITLE, SqliteSchema.COL_CONTENT, SqliteSchema.COL_EVALUATION,
                SqliteSchema.COL_DATE, SqliteSchema.COL_MEMO_TYPE, SqliteSchema.COL_ACTUAL_ID,
    ...
    public void updateByPublicId(JdbcTemplate jdbcTemplate, String publicId,
            MemoCreateRequest request, Long actualId, Long targetId) {
        String sql = String.format(
                "UPDATE %s SET %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = ?, %s = datetime('now') WHERE %s = ? AND %s IS NULL",
                SqliteSchema.TABLE_MEMO, SqliteSchema.COL_IMPORTANCE, SqliteSchema.COL_TITLE,
                SqliteSchema.COL_CONTENT, SqliteSchema.COL_EVALUATION, SqliteSchema.COL_DATE,
                SqliteSchema.COL_MEMO_TYPE, SqliteSchema.COL_ACTUAL_ID, SqliteSchema.COL_TARGET_ID,
    ...
    public void deleteByPublicId(JdbcTemplate jdbcTemplate, String publicId) {
        String sql = String.format("UPDATE %s SET %s = datetime('now') WHERE %s = ? AND %s IS NULL",
                SqliteSchema.TABLE_MEMO, SqliteSchema.COL_DELETED_AT, SqliteSchema.COL_PUBLIC_ID,
                SqliteSchema.COL_DELETED_AT);
