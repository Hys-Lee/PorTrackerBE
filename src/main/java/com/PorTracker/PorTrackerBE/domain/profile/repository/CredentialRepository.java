...
public class CredentialRepository {
    ...
    private final RowMapper<CredentialRecord> credentialMapper = (rs, rowNum)->CredentialRecord.builder().id(UUID.fromString(rs.getString("id"))).provider(rs.getString("provider")).accessToken(rs.getString("access_token")).refreshToken(rs.getString("refresh_token")).updatedAt(rs.getObject("updated_at", OffsetDateTime.class)).build();
    ...
    public Optional<CredentialRecord> findByUserId(UUID userId){
        String sql =String.format( 
            ...
            " WHERE %s=?",
            // select
        CentralSchema.COL_ID, CentralSchema.COL_PROVIDER, CentralSchema.COL_ACCESS_TOKEN, CentralSchema.COL_REFRESH_TOKEN, CentralSchema.COL_UPDATED_AT,
            // where
            CentralSchema.COL_ID
            );

        return jdbcTemplate.query(sql, ps->ps.setObject(1, userId), credentialMapper).stream().findFirst();

    }

    public void updateAccessToken(UUID userId, String newAccessToken){
        String sql = String.format("UPDATE public.credential SET %s = ?, %s = NOW()"+" WHERE %s = ?", 
        // update
        CentralSchema.COL_ACCESS_TOKEN, CentralSchema.COL_UPDATED_AT,
        // where
        CentralSchema.COL_ID
    );
