...
public class TargetPortfolioService {
        ...
        public List<com.PorTracker.PorTrackerBE.domain.target_portfolio.dto.TargetPortfolioData> getAllTargetPortfoliosFullData(
                ...
                List<TargetPortfolioRecord> portfolios =
                                targetPortfolioRepository.findAll(jdbcTemplate);
                ...
                List<Long> portfolioIds =
                                portfolios.stream().map(TargetPortfolioRecord::getId).toList();

                // 3. 모든 포트폴리오의 최신 아이템 한 번에 조회 (Map<PortfolioId, List<Item>>)
                java.util.Map<Long, List<TargetPortfolioItemRecord>> itemsMap = itemRepository
                                .findLatestItemsByPortfolioIds(jdbcTemplate, portfolioIds);

                // 4. 데이터 조립
                return portfolios.stream().map(
                                portfolio -> new com.PorTracker.PorTrackerBE.domain.target_portfolio.dto.TargetPortfolioData(
                                                portfolio,
                                                itemsMap.getOrDefault(portfolio.getId(),
        ...
        public com.PorTracker.PorTrackerBE.domain.target_portfolio.dto.TargetPortfolioData getTargetPortfolioDetail(
                ...
                List<TargetPortfolioItemRecord> items = itemRepository
                                .findItemsByLatestSnapshot(jdbcTemplate, portfolio.getId());
        ...
        public String addTargetPortfolio(String userId, TargetPortfolioCreateRequest request) {
                ...
                // 1. 포트폴리오 생성
                Long portfolioId = targetPortfolioRepository.save(jdbcTemplate, request.getName(),
                                request.getDate(), publicId);
