...
public class TargetPortfolioService {
    ...
    public List<com.PorTracker.PorTrackerBE.domain.target_portfolio.dto.TargetPortfolioData> getAllTargetPortfoliosFullData(
        ...
        return portfolios.stream().map(
                portfolio -> new com.PorTracker.PorTrackerBE.domain.target_portfolio.dto.TargetPortfolioData(
                        portfolio, itemsMap.getOrDefault(portfolio.getId(),
     ...
     */
    public List<TargetPortfolioItemRecord> getLatestSnapshotItems(String userId, String publicId) {
        JdbcTemplate jdbcTemplate = sqliteManager.getJdbcTemplateOfDataSource(userId);
    ...
    private void saveItems(JdbcTemplate jdbcTemplate, String userId, Long snapshotId,
        ...
        for (TargetPortfolioItemRequest item : items) {
            AssetRecord asset = assetService.getAssetByPublicId(userId, item.getAssetId());
            itemRepository.save(jdbcTemplate, snapshotId, asset.getId(), item.getCurrentRatioBp(),
