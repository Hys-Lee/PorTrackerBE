...
public class TargetPortfolioRepository {
        ...
        private static final String BASE_SELECT_SQL = String.format(
                        " SELECT %s, %s, %s, %s, %s, %s FROM %s", SqliteSchema.COL_ID,
                        SqliteSchema.COL_PUBLIC_ID, SqliteSchema.COL_NAME, SqliteSchema.COL_DATE,
                        SqliteSchema.COL_CREATED_AT, SqliteSchema.COL_DELETED_AT,
                        SqliteSchema.TABLE_TARGET_PORTFOLIO);

        private final RowMapper<TargetPortfolioRecord> portfolioMapper =
                        (rs, rowNum) -> TargetPortfolioRecord.builder()
        ...
        public Optional<TargetPortfolioRecord> findByPublicId(JdbcTemplate jdbcTemplate,
                        String publicId) {
                String sql = BASE_SELECT_SQL + " WHERE " + SqliteSchema.COL_PUBLIC_ID + "=? AND "
                                + SqliteSchema.COL_DELETED_AT + " IS NULL";
                return jdbcTemplate.query(sql, ps -> ps.setString(1, publicId), portfolioMapper)
                                .stream().findFirst();
        }

        public Long save(JdbcTemplate jdbcTemplate, String name, String date, String publicId) {
                String sql = String.format("INSERT INTO %s (%s, %s, %s) VALUES (?, ?, ?)",
                                SqliteSchema.TABLE_TARGET_PORTFOLIO, SqliteSchema.COL_PUBLIC_ID,
                                SqliteSchema.COL_NAME, SqliteSchema.COL_DATE);
        ...
        public void updateByPublicId(JdbcTemplate jdbcTemplate, String publicId, String name,
                ...
                String sql = String.format(
                                ...
                                SqliteSchema.TABLE_TARGET_PORTFOLIO, SqliteSchema.COL_NAME,
                                SqliteSchema.COL_DATE, SqliteSchema.COL_UPDATED_AT,
        ...
        public void deleteByPublicId(JdbcTemplate jdbcTemplate, String publicId) {
                String sql = String.format(
                                ...
                                SqliteSchema.TABLE_TARGET_PORTFOLIO, SqliteSchema.COL_DELETED_AT,
                                SqliteSchema.COL_PUBLIC_ID, SqliteSchema.COL_DELETED_AT);
