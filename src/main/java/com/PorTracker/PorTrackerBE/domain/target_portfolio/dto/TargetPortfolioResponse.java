...
public record TargetPortfolioResponse(
        @JsonProperty("id") String publicId,
        String name,
        String date,
        String createdAt,
        List<TargetPortfolioItemResponse> items) {

    public static TargetPortfolioResponse from(
            TargetPortfolioRecord record, List<TargetPortfolioItemRecord> items) {
        List<TargetPortfolioItemResponse> itemResponses =
                items.stream().map(TargetPortfolioItemResponse::from).toList();
        return new TargetPortfolioResponse(
                record.getPublicId(),
                record.getName(),
                record.getDate(),
                record.getCreatedAt(),
    ...
    public record TargetPortfolioItemResponse(String assetId, Long currentRatioBp
        ...
        public static TargetPortfolioItemResponse from(TargetPortfolioItemRecord item) {
            return new TargetPortfolioItemResponse(
                    item.getAssetPublicId(), item.getCurrentRatioBp()
