...
public class TargetPortfolioItemRepository {
    ...
    private final RowMapper<TargetPortfolioItemRecord> itemMapper =
            (rs, rowNum) -> TargetPortfolioItemRecord.builder()
                    ...
                    .currentRatioBp(rs.getLong(SqliteSchema.COL_CURRENT_RATIO_BP))
                    .ratioDeltaBp(rs.getLong(SqliteSchema.COL_RATIO_DELTA_BP)).build();

    public void save(JdbcTemplate jdbcTemplate, Long snapshotId, Long assetId, Long currentRatioBp,
            Long ratioDeltaBp) {
    ...
    /**
     * 특정 포트폴리오의 최신 스냅샷에 속한 아이템들을 JOIN으로 한번에 조회. target_portfolio_item → target_portfolio_snapshot →
     * target_portfolio 순으로 JOIN하고, asset 테이블도 JOIN하여 asset의 public_id를 함께 가져옴.
     */
    public List<TargetPortfolioItemRecord> findItemsByLatestSnapshot(JdbcTemplate jdbcTemplate,
            Long portfolioId) {
        String sql = String.format(
                "SELECT tpi.%s, tpi.%s, tpi.%s, tpi.%s, a.%s as %s" + " FROM %s tpi"
                        + " JOIN %s a ON a.%s = tpi.%s" + " WHERE tpi.%s = ("
                        + "   SELECT tps.%s FROM %s tps" + "   WHERE tps.%s = ?"
                        + "   ORDER BY tps.%s DESC LIMIT 1" + " )",
                // SELECT
                SqliteSchema.COL_ASSET_ID, SqliteSchema.COL_SNAPSHOT_ID,
                SqliteSchema.COL_CURRENT_RATIO_BP, SqliteSchema.COL_RATIO_DELTA_BP,
                SqliteSchema.COL_PUBLIC_ID, assetPublicIdName,
                // FROM target_portfolio_item
                SqliteSchema.TABLE_TARGET_PORTFOLIO_ITEM,
                // JOIN asset
                SqliteSchema.TABLE_ASSET, SqliteSchema.COL_ID, SqliteSchema.COL_ASSET_ID,
    ...
    public List<TargetPortfolioItemRecord> findItemsBySnapshotId(JdbcTemplate jdbcTemplate,
        ...
        String sql = String.format(
                ...
                SqliteSchema.COL_ID, SqliteSchema.COL_ASSET_ID, SqliteSchema.COL_SNAPSHOT_ID);
    ...
    public java.util.Map<Long, List<TargetPortfolioItemRecord>> findLatestItemsByPortfolioIds(
        ...
        // IN 절 동적 생성
        String inSql = String.join(",", java.util.Collections.nCopies(portfolioIds.size(), "?"));

        String sql = String.format(
                "SELECT tpi.%s, tpi.%s, tpi.%s, tpi.%s, a.%s as %s, tps.%s" + " FROM %s tpi"
                        + " JOIN %s tps ON tpi.%s = tps.%s" + " JOIN %s a ON a.%s = tpi.%s"
                        + " WHERE tps.%s IN (" + "   SELECT MAX(id) FROM %s" + "   WHERE %s IN (%s)"
                        + "   GROUP BY %s" + " )",
                // SELECT
                SqliteSchema.COL_ASSET_ID, SqliteSchema.COL_SNAPSHOT_ID,
                SqliteSchema.COL_CURRENT_RATIO_BP, SqliteSchema.COL_RATIO_DELTA_BP,
                SqliteSchema.COL_PUBLIC_ID, assetPublicIdName, SqliteSchema.COL_PORTFOLIO_ID, // 그룹핑을
                                                                                              // 위해
                                                                                              // 추가
                                                                                              // 조회
                // FROM & JOIN
                SqliteSchema.TABLE_TARGET_PORTFOLIO_ITEM,
                SqliteSchema.TABLE_TARGET_PORTFOLIO_SNAPSHOT, SqliteSchema.COL_SNAPSHOT_ID,
                SqliteSchema.COL_ID, SqliteSchema.TABLE_ASSET, SqliteSchema.COL_ID,
                SqliteSchema.COL_ASSET_ID,
                // WHERE IN Subquery
                SqliteSchema.COL_ID, SqliteSchema.TABLE_TARGET_PORTFOLIO_SNAPSHOT,
                SqliteSchema.COL_PORTFOLIO_ID, inSql, SqliteSchema.COL_PORTFOLIO_ID);
        ...
        return jdbcTemplate.query(sql, (java.sql.PreparedStatement ps) -> {
            ...
            java.util.Map<Long, List<TargetPortfolioItemRecord>> map = new java.util.HashMap<>();
            while (rs.next()) {
                ...
                map.computeIfAbsent(pId, k -> new java.util.ArrayList<>()).add(item);
            }
            return map;
        });
    }
