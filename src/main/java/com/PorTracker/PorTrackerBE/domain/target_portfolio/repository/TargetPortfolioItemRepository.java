...
public class TargetPortfolioItemRepository {
    ...
    private final RowMapper<TargetPortfolioItemRecord> itemMapper =
            (rs, rowNum) ->
                    TargetPortfolioItemRecord.builder()
     ...
     */
    public List<TargetPortfolioItemRecord> findItemsByLatestSnapshot(
            JdbcTemplate jdbcTemplate, Long portfolioId) {
        String sql =
                String.format(
                        "SELECT tpi.%s, tpi.%s,  tpi.%s, a.%s as %s"
                                + " FROM %s tpi"
                                + " JOIN %s a ON a.%s = tpi.%s"
                                + " WHERE tpi.%s = ("
                                + "   SELECT tps.%s FROM %s tps"
                                + "   WHERE tps.%s = ?"
    ...
    public List<TargetPortfolioItemRecord> findItemsBySnapshotId(
            JdbcTemplate jdbcTemplate, Long snapshotId) {
        String sql =
                String.format(
                        "SELECT tpi.%s, tpi.%s, tpi.%s,  a.%s as %s"
                                + " FROM %s tpi"
    ...
    public java.util.Map<Long, List<TargetPortfolioItemRecord>> findLatestItemsByPortfolioIds(
        ...
        String sql =
                String.format(
                        "SELECT tpi.%s, tpi.%s, tpi.%s, a.%s as %s, tps.%s"
                                + " FROM %s tpi"
        ...
        return jdbcTemplate.query(
                ...
                (java.sql.ResultSet rs) -> {
                    ...
                    while (rs.next()) {
                        ...
                        TargetPortfolioItemRecord item =
                                TargetPortfolioItemRecord.builder()
                                        .assetId(rs.getLong(SqliteSchema.COL_ASSET_ID))
                                        .assetPublicId(rs.getString(assetPublicIdName))
                                        .snapshotId(rs.getLong(SqliteSchema.COL_SNAPSHOT_ID))
                                        .currentRatioBp(
                                                rs.getLong(SqliteSchema.COL_CURRENT_RATIO_BP))
